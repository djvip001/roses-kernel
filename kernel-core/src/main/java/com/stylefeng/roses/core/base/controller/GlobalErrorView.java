		...
		if (map != null && map.get("code") != null && map.get("message") != null) {
			...
			if (map != null && map.get("          status") != null && map.get("error") != null) {
				...
				httpServletResponse.getWriter()
						.write(JSON.toJSONString(new ErrorResponseData((Integer) status, (String) error)));
			} else {
				httpServletResponse.getWriter()
						.write(JSON.toJSONString(new ErrorResponseData(CoreExceptionEnum.PAGE_NULL.getCode(),
								CoreExceptionEnum.PAGE_NULL.getMessage())));
			}
		}
	}
}
